...
\label{sec:functionality}
	...
	No Firebase, esse controle de segurança e regras é implementado usando um conjunto de regras definidas pelo programador através de descritores JSON. Estas são aplicadas a cada operação de leitura ou escrita, fazendo com que uma requisição seja valida somente se essas regras permitirem.
	\cite{https://firebase.google.com/docs/database/security/}. 
	...
	\label{sub:api}
	% Definição
	possibilidade de o desenvolvedor implementar sua propria api sobre o serviço contratado.
	...
	SaaS investem em infraestrutura para mantem o serviço escalavel e disponivel na medida que a demanda dos usuarios vai crescendo, deixando o programador responsavel por estruturar os dados e as consultas de forma a minimizar a quantidade requisições e quantidade de nós a serem acessadas para realizar uma consulta. 

	%como firebase resolve problemas de escalabilidade 
	No Firebase, os problemas de usuarios simultaneos e escalabilidade são resolvidos pela plataforma, porem a um custo que deve ser acertado entre as partes contratantes do serviço e o fornecedor. 

	% transcrever ideia do paragrafo abaixo.
	O problema se que pode ocorrer é caso um dos nós passa a ter mais de 1--10+ milhões de filhos (digamos que você estaja escrevendo um app de mensagens e colocou todas as mensagens de um certo periodo em um mesmo ramo `\textit{/root/messages}') Uma solução para esse problema seria estruturar seus dados de forma que impeça isso de acontecer (por exemplo, arquivando chats que são mais antigos que 1 dia, uma semana, um mes). A razão para essa limitação é que o Firebase é armazenado em memoria ``quente'' então quando se tenta acessar um nó, todos os filhos são baixados e isso acaba lotando a memoria causando uma queda no desempenho. 
	%\cite{https://groups.google.com/forum/#!topic/firebase-angular/OY7xtpoc9XA}

	% transcrever ideia do paragrafo abaixo.
	`` Some other things to consider when scaling your Firebase are your typical access patterns. For instance, in order to generate a list of users online are you iterating through all of the children in a users node and checking some field to see if they are online, or have you denormalized your data some and are storing that in a separate users-online node, which reduces the bandwidth used since you're getting exactly the information you want, rather than deriving it from other data. One of our mottos at Firebase is that disk space is cheap, your time is not. Obviously, this denormalization has it's tradeoffs, but we leave these choices up to our developers, rather than corralling them into one option, and we strive to provide adequate documentation on Firebase best practices to hopefully make these well known to our developer community.'' 
	%\cite{https://groups.google.com/forum/#!topic/firebase-angular/OY7xtpoc9XA}
